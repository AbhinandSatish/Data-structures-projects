...
bool checkDriverFound(std::vector<Driver>& driver, std::string& input){
    for(unsigned int i = 0; i < driver.size(); i ++) {
        cout << "is this function working"<< endl;
...
void parseDriverFile(const std::string& filename, std::vector<Driver>& drivers){
    ...
    // Debug statements
    // std::cout << "Number of drivers parsed: " << drivers.size() << std::endl;
    // for (const auto& driver : drivers) {
    //     std::cout << "Driver: " << driver.getDriverFirstName() << " " << driver.getDriverLastName() << driver.getDriver_Phone() << endl;
    // }
}
...
void writeCancel(const std::string& driversFile, const std:: string& ridersFile,
                ...
                std::vector<Driver>& drivers
                    ...
                    for (Driver& driver : drivers) {
                        cout << driver.getDriver_Phone() << endl;
                }
    ...
    std::ofstream output2(outputFile2);
        ...
        //cout << "Driver phone number" << closestDriver.getDriver_Phone() << endl;
        for (Driver driver : drivers) {
            //if driver is found
    ...
    //check if the cancel request is make by a rider or a driver
    bool request_rider = false;
    bool request_driver = false;

    cout << Riderobject.getRider_Phone() << endl;
    cout << RiderPhoneNumber << "argument" << endl;
    if(Riderobject.getRider_Phone() == RiderPhoneNumber){
        request_rider = true;
        
    }
    
    cout << "driver test"<< closestDriver.getDriver_Phone() <<"   "<< RiderPhoneNumber << endl;
    cout << "rider phone number" << Riderobject.getRider_Phone() << endl;

    if(closestDriver.getDriver_Phone() == RiderPhoneNumber){
        request_driver = true;
    }

    //f the canceling request is issued by a rider whose state is NOT Driver_on_the_way
    if(Riderobject.getRiderCurrentState() != "Driver_on_the_way" && request_rider == true){
        output0 << "You can only cancel a ride request if your driver is currently on the way to the pickup location." << std::endl;
    }
    //if the canceling request is issued by a driver whose state is NOT On_the_way_to_pickup
    else if(closestDriver.getDriverstate() != "On_the_way_to_pickup" && request_driver == true){
        output0 << "You can only cancel a ride request if your driver is currently on the way to the pickup location." << std::endl;
    }else if(Riderobject.getRiderCurrentState()== "Driver_on_the_way") {
        output0 << "Ride request for rider " << Riderobject.getRiderFirstName() << " is now canceled by the rider." << std::endl;
    }

    else if(closestDriver.getDriverstate() == "On_the_way_to_pickup"){
        output0<< "Your driver" << closestDriver.getDriverFirstName() << "has cancelled the ride request. We will now find a new driver for you." << std::endl;
        output0<< "Ride requested for rider " << Riderobject.getRiderFirstName() << ", looking for a " << Riderobject.getRiderVehiclePreference() << " vehicle." << std::endl;
        output0<< "Pick Up Location: " << Riderobject.getRiderPickupLocation() << ", Drop Off Location: " << Riderobject.getRiderDropoffLocation() << "." << std::endl;
        output0<< "We have found the closest driver " << Riderobject.getRassignedDriverFirstName() << "(" << closestDriver.getDriverRating() << ") for you." << std::endl;
        output0<< Riderobject.getRassignedDriverFirstName() << " is now " << distanceMiles << " miles away from you." << std::endl;
    }

    //now to print an updated version of drivers.txt to output1.txt  should be changed from On_the_way_to_pickup to Available, and the last 3 fields of the driver should be reset to null, meaning that this driver is now not associated with any rider
     for (Driver driver : drivers) {
        if (driver.getDriver_Phone() == RiderPhoneNumber && driver.getDriverstate() == "On_the_way_to_pickup") {
            cout << "test6  "<< endl;
            driver.setDriverstate("Available");
            driver.setDassignedRiderFirstName("null");
            driver.setDassignedRiderLastName("null");
            driver.setDassignedRiderPhoneNumber("null");
            break;
        }
    }

    // Write the updated drivers vector to output1.txt
    for (Driver driver : drivers) {
        output1 << driver.getDriverFirstName() << " " << driver.getDriverLastName() << " "<< driver.getDriverGender()<< " "<< driver.getDriverage()<< " "<<
        driver.getDriver_Phone()<< " " <<driver.getDriverRating() << " " << driver.getDriverLatitude() << " " << driver.getDriverLongitude() << " " << 
        driver.getVehicleType() << " " << driver.getDriverstate() << " " << driver.getDassignedRiderFirstName() << " " << driver.getDassignedRiderLastName() << " " << driver.getDassignedRiderPhoneNumber() << std::endl;
...
int main(int argc, char* argv[]){
    ...
    std::vector<Driver> drivers;
    parseDriverFile(driversFile, drivers);
    //printing all the drivers from the vector to test
    cout << "are you getting heree"<< endl;
    Driver driver;
    
    // for (const auto& driver : drivers) {
    //     cout << driver.getDriverFirstName() << driver.getDriver_Phone() << endl;
    // }

    
    //drivers = parseDriverFile(driversFile, drivers);
    //finding the information about the driver from the number provided from the command line if it is a cancel request 
    bool driver_found = false;
    for (unsigned int i = 0; i < drivers.size(); i++) {
        if (drivers[i].getDriver_Phone() == RiderPhoneNumber) {
            driver = drivers[i];
            bool driver_found = true;
            //cout << RiderPhoneNumber << endl;
            //cout<< driver.getDassignedRiderFirstName() << driver.getDriver_Phone() << endl;
            break;
        }
    }
    cout << "driver found test"<<driver_found << endl;
    drivers = {};
    ...
    if(!driver_found && check){
        output0 << "Account does not exist." << std::endl;
    }

    
    if(Request == "request" && rider_found == true){
    ...
    if(Request == "cancel"){
        ...
        writeCancel(driversFile, ridersFile, outputFile0, outputFile1, outputFile2, RiderPhoneNumber, drivers);
    }
